...
app.get('/', (req, res) => {
  res.json({ 
    message: 'Battleship Game Server',
    status: 'running',
    timestamp: new Date().toISOString()
  });
});
...
function matchPlayers() {
  while (gameState.waitingPlayers.length >= 2) {
    ...
    console.log(`Matched players: ${gameState.players[player1Id].name} vs ${gameState.players[player2Id].name}`);
  }
}
...
server.listen(PORT, () => {
  console.log(`🚀 Simple Battleship server running on port ${PORT}`);
